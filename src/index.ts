...
import { BunshinConfig, InitOptions, SendOptions, SlackUser } from "./types.js";
...
function loadConfig(): BunshinConfig {
  ...
  if (existsSync(localRcPath)) {
    try {
      const localConfig = JSON.parse(readFileSync(localRcPath, "utf8")) as Partial<BunshinConfig>;
      ...
      console.warn(
        chalk.yellow(
          `Warning: Invalid .bunshinrc file in current directory: ${(error as Error).message}`,
        ),
      );
    }
  }
  ...
  if (existsSync(homeRcPath)) {
    try {
      const homeConfig = JSON.parse(readFileSync(homeRcPath, "utf8")) as Partial<BunshinConfig>;
      ...
      console.warn(
        chalk.yellow(
          `Warning: Invalid .bunshinrc file in home directory: ${(error as Error).message}`,
        ),
      );
    }
  }
...
const slack = new WebClient(config.slackToken || undefined);
...
program
  ...
  .action(async (options: InitOptions) => {
    try {
      ...
      let currentConfig: Partial<BunshinConfig> = {};
      if (existsSync(configPath)) {
        try {
          ...
          console.warn(
            chalk.yellow(
              `Warning: Invalid existing .bunshinrc file: ${(error as Error).message}`,
            ),
          );
        }
      }
  ...
  .action(async () => {
    try {
      ...
      try {
        ...
        console.error(
          chalk.red(`Slack API Error: ${(error as any).data?.error || (error as Error).message}`),
        );

        if (
          (error as any).data?.error === "invalid_auth" ||
          (error as any).data?.error === "token_expired"
        ) {
          console.log(
...
program
  ...
  .action(async (message: string, options: SendOptions) => {
    try {
      ...
      async function processUserMentions(text: string): Promise<string> {
        ...
        try {
          ...
          const users = userListResponse.members as SlackUser[];
          ...
          console.warn(
            chalk.yellow(
              `Warning: Couldn't process user mentions: ${(error as Error).message}`,
            ),
          );
        }
      ...
      console.error(chalk.red("Error sending message:"), (error as Error).message);
      if ((error as any).data?.error) {
        console.error(chalk.red(`Slack API Error: ${(error as any).data.error}`));
      }
      process.exit(1);
    }
  });
