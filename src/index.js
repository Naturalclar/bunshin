...
function loadConfig() {
  ...
  if (existsSync(localRcPath)) {
    try {
      ...
      if (localConfig.defaultChannel) config.defaultChannel = localConfig.defaultChannel;
    } catch (error) {
      console.warn(chalk.yellow(`Warning: Invalid .bunshinrc file in current directory: ${error.message}`));
    }
  }
  ...
  if (existsSync(homeRcPath)) {
    try {
      ...
      if (homeConfig.defaultChannel) config.defaultChannel = homeConfig.defaultChannel;
    } catch (error) {
      console.warn(chalk.yellow(`Warning: Invalid .bunshinrc file in home directory: ${error.message}`));
    }
  }
    ...
    try {
      ...
      if (existsSync(configPath)) {
        try {
          ...
          console.warn(chalk.yellow(`Warning: Invalid existing .bunshinrc file: ${error.message}`));
        }
      }
      ...
      if (!currentConfig.slackToken) {
        console.log(chalk.yellow('No Slack token provided. You can add it later with:'));
        console.log(chalk.blue(`bunshin init --token xoxp-your-token`));
        console.log(chalk.yellow('Remember to use a User Token (xoxp-) to send messages as yourself, not a Bot Token.'));
      }
    } catch (error) {
  ...
  .action(async () => {
    try {
      if (!config.slackToken) {
        console.error(chalk.red('Error: Slack token not found in configuration'));
        console.log(chalk.yellow('Please run "bunshin init --token xoxp-your-token" to set up your token'));
        process.exit(1);
      }
      ...
      try {
        ...
        if (config.slackToken.startsWith('xoxp-')) {
          console.log(chalk.green('✓ Using a User Token - messages will be sent as yourself'));
        } else {
          console.log(chalk.yellow('⚠️ Warning: Not using a User Token (xoxp-) - messages may appear as a bot'));
          console.log(chalk.yellow('   To send messages as yourself, use a User Token that starts with xoxp-'));
        }
        ...
        if (config.defaultChannel) {
          ...
          console.log(chalk.yellow('No default channel configured. You will need to specify a channel with each message.'));
        }
      } catch (error) {
        console.error(chalk.red('Error: Failed to authenticate with Slack'));
        console.error(chalk.red(`Slack API Error: ${error.data?.error || error.message}`));

        if (error.data?.error === 'invalid_auth' || error.data?.error === 'token_expired') {
          console.log(chalk.yellow('Your token appears to be invalid or expired.'));
...
program
  ...
  .option('-c, --channel <channel>', 'Channel to send message to', config.defaultChannel)
  .action(async (message, options) => {
    try {
      if (!config.slackToken) {
        console.error(chalk.red('Error: Slack token not found in configuration'));
        console.log(chalk.yellow('Please run "bunshin init --token xoxp-your-token" to set up your token'));
        console.log(chalk.yellow('Or create a .bunshinrc file in your home or project directory'));
        console.log(chalk.yellow('Make sure to use a User Token (xoxp-) to send messages as yourself, not a Bot Token.'));
        process.exit(1);
      }

      if (!options.channel) {
        console.error(chalk.red('Error: Channel not specified'));
        console.log(chalk.yellow('Please specify a channel with --channel or set a default channel in your config'));
        process.exit(1);
      }

      console.log(chalk.blue(`Sending message to channel ${options.channel}...`));
